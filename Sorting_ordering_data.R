...
# TASK day3: Create plots using plot and ggplot syntax. Find out more information
# about our dataset!
# for example: 
...
df_agg_type$ProductionTypeName <- factor(df_agg_type$ProductionTypeName,
                                         levels = df_agg_type$ProductionTypeName)
head(df_agg_type$ProductionTypeName)
# or: in aes(y = reorder(ProductionTypeName, -InstalledGenCapacity_sum))

ggplot() +
  geom_bar(data = df_agg_type, 
           aes(x = InstalledGenCapacity_sum,
               y = reorder(ProductionTypeName, InstalledGenCapacity_sum),
